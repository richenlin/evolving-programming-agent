...
| 子命令 | 对应脚本操作 | 说明 |
|--------|------------|------|
| `on` | `--on` | 仅开启进化模式（不输出欢迎语） |
| `off` | `--off` | 关闭进化模式 |
| `status` | `--status` | 查看当前状态（激活/未激活） |
| `init` | `--init` | 强制重新初始化（同无参数运行） |
...
2. **智能调度**:
   - 输入 "帮我写个登录页面" -> 调度 `programming-assistant`
   - 输入 "学习这个仓库..." -> 调度 `github-to-skills`
   - 输入 "保存这次的经验" -> 调度 `knowledge-base`
3. **进化闭环**:
