...
| 核心依赖 | Python >= 3.8, PyYAML; 可选: jieba                                        |
...
| Agent        | 模型              | 权限        | 文件                          |
| ------------ | ----------------- | ----------- | ----------------------------- |
| orchestrator | 继承主 agent 模型 | SKILL.md 主进程 | SKILL.md（agents/orchestrator.md 保留备选） |
...
SKILL.md 作为 orchestrator 主进程包含完整的调度闭环（初始化→意图识别→子 agent 调度→验证）。工作流文件（simple-mode/full-mode）为 @coder 的执行指南。
3.4 总体符合度评分
| 维度             | 得分 (1-10) | 说明                     |
| ---------------- | ----------- | ------------------------ |
...
3. 四级知识检索
query.py 实现精确匹配(权重3) → 部分匹配(权重2) → 模糊匹配(SequenceMatcher, 阈值0.6) → BM25 语义搜索(内置零依赖)。综合相关性公式：trigger×0.4 + effectiveness×0.3 + recency×0.2 + usage×0.1。查询命中时自动更新 usage_count 和 last_used_at。
4. 知识库生命周期管理
lifecycle.py 实现 decay（90天未用衰减 effectiveness）和 gc（清理 effectiveness < 0.1 的条目）。防止知识库无限膨胀。dashboard.py 提供可视化统计。
5. 渐进披露文档架构
SKILL.md (orchestrator 主进程：初始化→意图→调度→验证)
  ├→ workflows/simple-mode.md 或 full-mode.md (@coder 工作指南)
  ├→ workflows/consult-mode.md (orchestrator 直接执行)
  └→ workflows/evolution-check.md (@evolver 执行)
...
| P2-6 | workflow 大量重复 | P2     | ✅ 已解决 | 自包含模式文件 + 删除 _base.md |
