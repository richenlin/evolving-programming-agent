...
```
用户请求 → [异步]知识检索 → 场景识别 → 选择模式 → 执行任务 → 会话结束 → [异步]知识归纳
              │                                                         │
              └─► .knowledge-context.md                                  └─► 知识库更新
```
...
Task(
    ...
    prompt="""
    执行知识检索，将结果写入 .knowledge-context.md:
...
主任务执行过程中，可读取 `.knowledge-context.md` 获取经验参考：

```bash
cat .knowledge-context.md 2>/dev/null
```

知识上下文包含：
- **高相关度知识**: 直接相关的经验、解决方案、最佳实践
- **可能相关**: 相关的设计模式、技术栈知识

### 任务结束：知识归纳

任务完成后，**异步启动**知识归纳子会话，提取有价值的经验：
...
```
┌─────────────────────────────────────────────────────────────────┐
│ 0. 检查知识 → 读取 .knowledge-context.md 获取相关经验            │
│ 1. 分析问题 → 识别关键约束和目标                                 │
│ 2. 生成方案 → 至少2-3个可行方案（参考知识库经验）                 │
...
| 知识归纳 | 检测进化触发条件，异步归纳知识 |

### 进化触发条件

以下情况触发知识归纳（异步执行，不阻塞用户）：
...
```bash
# 根据输入检索
python knowledge-base/scripts/knowledge_trigger.py --input "问题描述" --format context

# 结合项目检测
python knowledge-base/scripts/knowledge_trigger.py --input "问题描述" --project . --format context
```
